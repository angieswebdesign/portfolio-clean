...
export function openModal({ data = null, mode = "panel", src = null }) {
  ...
  ? `
    <div id="layer-2-lightbox" class="lightbox-frame">
      <div class="lightbox-media">
        <img id="lightbox-image" src="${src}" alt="">
      </div>
    </div>
  `
  ...
  if (mode === "lightbox") {
    ...
    const frame = document.getElementById("layer-2-lightbox");
  ...
  // NOTE:
  // Directional reveal (left-to-right vs right-to-left) intentionally simplified.
  ...
  if (mode === "panel") {
    requestAnimationFrame(() => {
      ...
      // 🔑 FORCE layout AFTER positioning
