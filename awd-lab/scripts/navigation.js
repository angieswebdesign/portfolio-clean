...
const FRAME_COLORS = [
  { r: 195, g: 195, b: 195 }, // React (grey)
  { r: 20,  g: 20,  b: 20 }, // Chart (black)
  { r: 195, g: 195, b: 195 }, // Surface (grey)
...
function goToIndex(nextIndex) {
  ...
  function animate(now) {
    ...
    console.log(
      `[NAV] progress: ${easedProgress.toFixed(4)}`
    );
...
const toggleItems = document.querySelectorAll('.toggle-switch li');
...
toggleItems.forEach((item, index) => {
  item.addEventListener('click', (e) => {
    ...
    wrappers.forEach((wrapper, i) => {
      wrapper.dataset.visible = i === index ? 'true' : 'false';
    });
  });
});